...
public class ServerManager implements MessageListener
{
    ...
    // for sending particular Client
   public void send_message_to_group(String message )
   {
	   //#GROUP dc hellowmygroup chatgroup bakr~beko~
       //tag from message groupname  groupmember
       StringTokenizer tokens=new StringTokenizer(message);
      // String to=tokens.nextToken();
       tokens.nextToken(); //tag
       tokens.nextToken(); //from
       tokens.nextToken(); //message
       tokens.nextToken(); //groupName
   String To=tokens.nextToken(); //group member
   StringTokenizer token_TO=new StringTokenizer(To,"~");

while (token_TO.hasMoreTokens()) {
	String  to = token_TO.nextToken();
